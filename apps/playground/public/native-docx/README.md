...
The browser decodes it before sending the exact DOCX bytes to the browser-local Go WASM engine by default. An explicitly selected server fallback uses `POST /v1/docx/extract` instead. It supports the guarded text write-back proof, but is not evidence of exact page layout or page paint.
