...
describe('playground workbench page contract', () => {
  ...
  it('wires the PDF workbench to browser-safe package APIs and a Vite-only Node host', () => {
    ...
    expect(page).toMatch(/if \(result\.applied > 0\) \{\s*pendingFormDrafts\.current = \{ bytes: result\.bytes, drafts: fields, skipped: result\.skipped \}\s*await applyBytes\(result\.bytes, message\)/)
    expect(page).toContain('pending?.bytes === bytes')
    expect(page).toContain('restorePdfSkippedDrafts(nextFields, pending.drafts, pending.skipped)')
