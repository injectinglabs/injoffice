...
describe('playground workbench design system', () => {
  ...
  it('uses a continuous showcase layout with independently identified live sections', () => {
    ...
    expect(app).toContain('{workspace.title}')
    expect(app).toContain('TOOL_WORKSPACES.map')
    ...
    expect(app).toContain("aria-current={surface === workspace.tool ? 'location' : undefined}")
