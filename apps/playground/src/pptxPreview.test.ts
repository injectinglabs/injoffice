...
describe('PPTX approximate preview policy', () => {
  ...
  it('uses the authored bullet, resolved font and paragraph offsets only in the labeled approximate view', () => {
    const deck: NativePptxDeck = {
      ...
      slides: [{ id: 'slide', provenance: 'authored', passthrough: [], compatibility: { status: 'editable', diagnostics: [] }, elements: [element({ kind: 'text', paragraphs: [{ bullet: true, bulletCharacter: '▪', marginLeftEmu: 12, indentEmu: -4, runs: [{ text: 'Author list', fontFamily: 'DejaVu Sans', fontSizeHundredthPt: 1200 }] }] })] }],
    }
    const html = renderToStaticMarkup(createElement(PptxFilePreview, { deck }))
