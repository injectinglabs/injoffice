...
export function NativeDocxPages({ bytes, packageDigest, apiBase }: { bytes: Uint8Array; packageDigest: string; apiBase: string }) {
  ...
  return <section className="docx-native-pages" aria-label="Native document pages">
    ...
    <p>Approximate pages may use current layout rules for eligible older Word settings and black automatic table borders on a source-qualified white background. Each applied policy is disclosed below. Other unsupported features remain refused; the original file is unchanged.</p>
