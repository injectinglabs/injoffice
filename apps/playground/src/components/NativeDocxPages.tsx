...
export function nativeDocxFontSubstitutionSummary(value:NativeDocxFontSubstitutionPreviewV1):string[]{
  const selections=[...new Set(value.substitutions.map(r=>`${r.source_family} → ${r.selected_family} (${r.weight}, ${r.style}).`))]
...
export function NativeDocxPages({ bytes, packageDigest, apiBase, contentPreview = true }: { bytes: Uint8Array; packageDigest: string; apiBase: string; contentPreview?: boolean }) {
  ...
  async function render(approximate = false, fontSubstitution = false, textboxes = false) {
    ...
    try {
      ...
      if(textboxes){
        const decoded=decodeTextboxesPageResponse(value,packageDigest)
        next={...decoded.body_paint,approximate:true,reasons:[decoded.warning,...decoded.source_diagnostics.map(d=>d.message)],textboxes:decoded.textboxes}
  ...
  return <section className="docx-native-pages" aria-label="Native document pages">
    ...
    <p>Approximate pages may use current layout rules for eligible older Word settings, an explicit 11 pt host default where the source has no font size, and black automatic table borders on a source-qualified white background. This does not reproduce the original pagination. Each applied policy is disclosed below. Other unsupported features remain refused; the original file is unchanged.</p>
