  ...
  function object(element: NativeElement) {
    ...
    return <g key={element.id} transform={`translate(${x * size.scale} ${y * size.scale})`}>
      ...
      {issue ? <>
        ...
        {text.length > 0 && <foreignObject width={cx * size.scale} height={cy * size.scale}>
          <div style={{ height: '100%', boxSizing: 'border-box', overflow: 'hidden', color: '#141816', display: 'flex', flexDirection: 'column', justifyContent: body?.verticalAnchor === 'center' ? 'center' : body?.verticalAnchor === 'bottom' ? 'flex-end' : 'flex-start', padding: `${(body?.topInsetEmu ?? 0) * size.scale}px ${(body?.rightInsetEmu ?? 0) * size.scale}px ${(body?.bottomInsetEmu ?? 0) * size.scale}px ${(body?.leftInsetEmu ?? 0) * size.scale}px` }}>
            {text.map((paragraph, paragraphIndex) => <p key={paragraphIndex} style={{ margin: 0, lineHeight: 1.2, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', textAlign: paragraph.align === 'center' ? 'center' : paragraph.align === 'right' ? 'right' : 'left' }}>
              {paragraph.bullet ? '• ' : ''}{paragraph.runs.map((run, runIndex) => <span key={runIndex} style={{ fontFamily: 'Arial, sans-serif', fontWeight: run.bold ? 700 : 400, fontStyle: run.italic ? 'italic' : 'normal', fontSize: (run.fontSizeHundredthPt ?? 1800) * 127 * size.scale, color: previewColor(run.color, '#141816') }}>{run.text}</span>)}
            </p>)}
          </div>
        </foreignObject>}
      </>}
