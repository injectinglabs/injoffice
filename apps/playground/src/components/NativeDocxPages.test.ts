...
describe('native document page viewer', () => {
  ...
  it('keeps the exact helper control name used by local capture', () => {
    const html = renderToStaticMarkup(createElement(NativeDocxPages, { bytes: new Uint8Array([1]), packageDigest: `sha256:${'c'.repeat(64)}`, apiBase: '.', contentPreview: false }))
    expect(html).toMatch(/<button[^>]*>Upload to helper and render native pages<\/button>/)
    expect(html).toContain('Nothing is uploaded until')
    expect(html).not.toContain('content preview below')
  })
  it('does not upload on render and names the actual destination', () => {
    ...
    try {
      ...
      expect(markup).toContain('does not reproduce the original pagination')
