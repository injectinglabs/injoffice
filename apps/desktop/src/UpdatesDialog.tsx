...
const labels: Record<DesktopUpdateState['status'], string> = {
  ...
  available: 'A new version is available', downloading: 'Getting your update ready…',
...
export function UpdateNotice({ onOpen }: { onOpen(): void }) {
  ...
  // The update arrives in the background, so the moment it is ready the dialog raises itself:
  // the only question left is whether to restart. It asks once per version, and a user who
  // closed it keeps working with the notice as the way back.
  const announced = useRef('');
  useEffect(() => {
    if (!state || state.status !== 'downloaded' || state.manualInstall) return;
    const version = String(state.version ?? '');
    if (announced.current === version) return;
