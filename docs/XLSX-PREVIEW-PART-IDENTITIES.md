...
This change only corrects transport validation. It does not broaden the supported spreadsheet features, establish formula-cache freshness or claim print fidelity. Tests exercise safe and unsafe identities across all five supplemental decoders, drawing sentinels and the complete public envelope. Actual Go-produced source/inspection envelopes and browser evidence remain in the local fidelity corpus, outside git and CI.
