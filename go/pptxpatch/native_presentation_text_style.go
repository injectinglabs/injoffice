...
// Only explicit matching level styles are qualified here. DefPPr fallback
// semantics differ in PowerPoint and remain refused, not merged by guesswork.
// Table cells keep their separate table-style path; no font defaults are invented.
func (extractor *nativeExtractor) extractNativeShapeParagraphs(body *nativeXMLNode, dialect nativeExtractDialect) ([]NativeParagraph, error) {
	defaults := extractor.presentationTextStyle
	if defaults == nil {
