...
export interface DocxWasmClient {
  inspectPartialContent(bytes:Uint8Array,options?:DocxWasmOperationOptions):Promise<{document:NativeDocxDocumentV1;resolved_layout:NativeDocxResolvedLayoutInputV1}>
...
class DocxWasmClientImpl implements DocxWasmClient {
  ...
  async inspectPartialContent(bytes:Uint8Array,options:DocxWasmOperationOptions={}):Promise<{document:NativeDocxDocumentV1;resolved_layout:NativeDocxResolvedLayoutInputV1}>{
    assertPackageSize(bytes,this.maxPackageBytes)
    ...
    try{
      ...
      if(!value||Object.keys(value).sort().join(',')!=='document,package_sha256,protocol,resolved_layout,version'||value.protocol!=='injoffice.docx.partial-source'||value.version!==1||value.package_sha256!==hash)throw new TypeError('Partial source package identity mismatch')
      ...
      return {document:document.value,resolved_layout:layout.value}
