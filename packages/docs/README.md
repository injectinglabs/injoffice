...
Native DOCX text shaping honors inherited `w:spacing` character tracking (ECMA-376 §17.3.2.35) in bounded signed whole twips (±31,680, i.e. ±1,584 pt). The resolved `letter_spacing_twips` measurement is added once at the trailing edge of every shaped cluster, the run's last cluster included, which is what Word's own PDF export does: in `StyleRef-DE.docx` the Subtitle style's `w:spacing w:val="15"` becomes a single `0.0651 Tc` and Word's advance for that run matches a per-cluster delta to within its own glyph-width rounding, against a full missing tracking step for a between-clusters-only delta. An authored `w:spacing w:val="0"` states the absence of its own effect and stays exactly as inert as an omitted element. Unit-bearing, signed-prefixed, zero-padded, fractional, duplicate and out-of-range measurements remain unqualified and source-preserved, as does any tracking whose cluster shapes to more than one glyph, where a cluster-edge delta and Word's per-glyph one are not the same count. Word spacing is still not modeled and still refuses.
