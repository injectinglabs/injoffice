...
const style = object({
  number_format: nullable({ type: 'string', maxLength: 255 }), font_name: nullable({ type: 'string', maxLength: 255 }),
  font_size_points: nullable({ type: 'number', minimum: 1, maximum: 409 }), bold: nullable({ type: 'boolean' }), italic: nullable({ type: 'boolean' }),
  font_color: nullable({ type: 'string', pattern: '^#[0-9A-Fa-f]{6}$' }), fill_color: nullable({ type: 'string', pattern: '^#[0-9A-Fa-f]{6}$' }),
  horizontal_alignment: nullable({ type: 'string', enum: ['general', 'left', 'center', 'right'] }), vertical_alignment: nullable({ type: 'string', enum: ['top', 'middle', 'bottom'] }),
  wrap_text: nullable({ type: 'boolean' }),
}, [])
...
function mutation(operation: AgentOperation): SupportedWorkbookMutation {
  ...
  switch (operation.name) {
    ...
    case 'xlsx.style.patch': { exact(input, ['sheetId', 'range', 'style']); if (!input.style || typeof input.style !== 'object' || Array.isArray(input.style)) throw new Error('style must be an object'); return { ...base, kind: 'style.patch', range: ref(input, 'range'), style: structuredClone(input.style) as StyleDelta } }
...
function operationPassed(workbook: XlsxNativeWorkbook, operation: SupportedWorkbookMutation): boolean {
  ...
  switch (operation.kind) {
    ...
    case 'style.patch': return [...sheet.cells].filter((cell) => cell.row >= operation.range.row && cell.row <= operation.range.end_row && cell.column >= operation.range.column && cell.column <= operation.range.end_column).every((cell) => { const style = workbook.styles.find((item) => item.id === cell.style_id)?.effective; return !!style && Object.entries(operation.style).every(([key, value]) => value === null || style[key as keyof typeof style] === value) })
  }
}
