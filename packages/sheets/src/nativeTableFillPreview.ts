...
export function nativeTableFillPreview(objects:NativeWorkbookObjectsV1,revision:string,sheetPart:string,row:number,column:number,fill:NativeWorkbookFillV2|undefined):string|undefined {
 ...
 if(!palette||row>bottom-table.total_rows)return undefined
...
export function nativeTableHeaderTextPreview(objects:NativeWorkbookObjectsV1,revision:string,sheetPart:string,row:number,column:number,fill:NativeWorkbookFillV2|undefined,styleID:number):boolean {
 const color=nativeTableFillPreview(objects,revision,sheetPart,row,column,fill)
