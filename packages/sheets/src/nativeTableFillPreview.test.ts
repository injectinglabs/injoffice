...
describe('native table fill preview',()=>{
 ...
 it('does not replace direct fills, absent provenance, stale results or ambiguous overlapping tables',()=>{
  for(const provenance of [undefined,{origin:'styles-record' as const,fill_id:2,color:'#FFFFFF'}])expect(nativeTableFillPreview(objects,revision,'xl/worksheets/sheet1.xml',1,1,provenance)).toBeUndefined()
  expect(nativeTableFillPreview(objects,'stale','xl/worksheets/sheet1.xml',1,1,fill)).toBeUndefined()
