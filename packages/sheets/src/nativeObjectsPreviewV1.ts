...
export interface NativeTablePreviewV1 {part:string;sheet_part:string;name:string;ref:string;style:string;header_rows:number;total_rows:number;row_stripes:boolean;column_stripes:boolean;warnings:string[];fill_preview?:{header:string;stripe:string;body:string;header_font_style_ids:number[]}}
...
export function decodeNativeWorkbookObjectsV1(input:unknown,packageSHA256:string):NativeWorkbookObjectsV1 {
 ...
 const tables=list(value.tables,64).map(v=>{const hasFill=!!v&&typeof v==='object'&&Object.hasOwn(v,'fill_preview');const t=obj(v,['part','sheet_part','name','ref','style','header_rows','total_rows','row_stripes','column_stripes','warnings',...(hasFill?['fill_preview']:[])]);let fill_preview:NativeTablePreviewV1['fill_preview'];if(hasFill){const p=obj(t.fill_preview,['header','stripe','body','header_font_style_ids']);const color=(v:unknown)=>typeof v==='string'&&/^#[0-9A-F]{6}$/.test(v)?v:fail();const ids=list(p.header_font_style_ids,4096).map(v=>typeof v==='number'&&Number.isSafeInteger(v)&&v>=0&&v<65536?v:fail());if(new Set(ids).size!==ids.length)return fail();fill_preview={header:color(p.header),stripe:color(p.stripe),body:color(p.body),header_font_style_ids:ids};if(t.style!=='TableStyleMedium2'||t.column_stripes!==false||!t.sheet_part)return fail()}return {part:part(t.part),sheet_part:part(t.sheet_part,true),name:text(t.name),ref:text(t.ref,64),style:text(t.style,256),header_rows:count(t.header_rows),total_rows:count(t.total_rows),row_stripes:bit(t.row_stripes),column_stripes:bit(t.column_stripes),warnings:warnings(t.warnings),...(fill_preview?{fill_preview}:{})}})
 let points=0
