...
export interface NativeTablePreviewV1 {part:string;sheet_part:string;name:string;ref:string;style:string;header_rows:number;total_rows:number;row_stripes:boolean;column_stripes:boolean;warnings:string[];number_formats?:NativeTableNumberFormatV1[];border_preview?:NativeTableBorderPreviewV1;fill_preview?:{header:string;stripe:string;body:string;header_font_style_ids:number[];fill_style_ids:number[]}}
export interface NativeTableBorderPreviewV1 {color:string;totals_color:string;width_points:1;totals_width_points:3;style_ids:number[]}
export interface NativeTableNumberFormatV1 {ref:string;dxf_id:number;number_format:string;style_ids:number[]}
...
export function decodeNativeWorkbookObjectsV1(input:unknown,packageSHA256:string):NativeWorkbookObjectsV1 {
 ...
 const value=obj(input,['protocol','version','package_sha256','tables','charts',...(hasRows?['row_geometry']:[])])
 ...
 let styleIDs=0,formatRegions=0
 const tables=list(value.tables,64).map(v=>{const hasBorders=!!v&&typeof v==='object'&&Object.hasOwn(v,'border_preview');const hasFormats=!!v&&typeof v==='object'&&Object.hasOwn(v,'number_formats');const hasFill=!!v&&typeof v==='object'&&Object.hasOwn(v,'fill_preview');const t=obj(v,['part','sheet_part','name','ref','style','header_rows','total_rows','row_stripes','column_stripes','warnings',...(hasFill?['fill_preview']:[]),...(hasFormats?['number_formats']:[]),...(hasBorders?['border_preview']:[])]);let fill_preview:NativeTablePreviewV1['fill_preview'];if(hasFill){const p=obj(t.fill_preview,['header','stripe','body','header_font_style_ids','fill_style_ids']);const color=(v:unknown)=>typeof v==='string'&&/^#[0-9A-F]{6}$/.test(v)?v:fail();const ids=list(p.header_font_style_ids,4096).map(v=>typeof v==='number'&&Number.isSafeInteger(v)&&v>=0&&v<65536?v:fail());const fillIDs=list(p.fill_style_ids,4096).map(v=>typeof v==='number'&&Number.isSafeInteger(v)&&v>=0&&v<65536?v:fail());styleIDs+=ids.length+fillIDs.length;if(styleIDs>16384||new Set(fillIDs).size!==fillIDs.length||new Set(ids).size!==ids.length)return fail();fill_preview={header:color(p.header),stripe:color(p.stripe),body:color(p.body),header_font_style_ids:ids,fill_style_ids:fillIDs};if(t.style!=='TableStyleMedium2'||t.column_stripes!==false||!t.sheet_part)return fail()}let border_preview:NativeTableBorderPreviewV1|undefined;if(hasBorders){const b=obj(t.border_preview,['color','totals_color','width_points','totals_width_points','style_ids']);const ids=list(b.style_ids,4096).map(v=>typeof v==='number'&&Number.isSafeInteger(v)&&v>=0&&v<65536?v:fail());styleIDs+=ids.length;if(styleIDs>16384||new Set(ids).size!==ids.length||!fill_preview||b.width_points!==1||b.totals_width_points!==3||typeof b.color!=='string'||!/^#[0-9A-F]{6}$/.test(b.color)||typeof b.totals_color!=='string'||!/^#[0-9A-F]{6}$/.test(b.totals_color))return fail();border_preview={color:b.color,totals_color:b.totals_color,width_points:1,totals_width_points:3,style_ids:ids}}const number_formats=hasFormats?list(t.number_formats,384).map(v=>{const f=obj(v,['ref','dxf_id','number_format','style_ids']);const ids=list(f.style_ids,4096).map(v=>typeof v==='number'&&Number.isSafeInteger(v)&&v>=0&&v<65536?v:fail());styleIDs+=ids.length;if(styleIDs>16384||++formatRegions>1024||new Set(ids).size!==ids.length||typeof f.dxf_id!=='number'||!Number.isSafeInteger(f.dxf_id)||f.dxf_id<0||f.dxf_id>65535)return fail();return {ref:text(f.ref,64),dxf_id:f.dxf_id,number_format:text(f.number_format),style_ids:ids}}):undefined;return {part:part(t.part),sheet_part:part(t.sheet_part,true),name:text(t.name),ref:text(t.ref,64),style:text(t.style,256),header_rows:count(t.header_rows),total_rows:count(t.total_rows),row_stripes:bit(t.row_stripes),column_stripes:bit(t.column_stripes),warnings:warnings(t.warnings),...(fill_preview?{fill_preview}:{}),...(number_formats?{number_formats}:{}),...(border_preview?{border_preview}:{})}})
 let points=0
 ...
 return {protocol:'injoffice.xlsx.preview-objects',version:1,package_sha256:packageSHA256,tables,charts,...(hasRows?{row_geometry:decodeNativeStoredRowGeometryV1(value.row_geometry)}:{})}
}
