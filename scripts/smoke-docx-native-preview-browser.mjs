...
try {
  ...
  await assert(`(() => { const page=window.__nativeDocxPaint.pages[1], ids=page.lines.filter(line=>line.region==='body').flatMap(line=>line.command_ids); const digits=page.commands.filter(command=>ids.includes(command.id)&&command.kind==='fill_glyph_path'&&[20,21,28].includes(command.glyph_id)).map(command=>command.glyph_id); return JSON.stringify(digits)===JSON.stringify([21,21]); })()`, 'body PAGE and NUMPAGES converge to page two of two without cached 999')
