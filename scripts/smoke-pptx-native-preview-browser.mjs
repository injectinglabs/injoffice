...
try{
 ...
 await assert(`window.__pptxPosts.length===5&&window.__pptxPosts[4].hash===${JSON.stringify(chartHash)}`,'chart preview uses exact original source bytes')
 ...
 await assert('window.__pptxPosts.length===5','replacement still requires consent');await clickRender()
