...
const root=resolve(import.meta.dirname,'..'),scratch=mkdtempSync(resolve(tmpdir(),'injoffice-literal-radar-browser-'))
const artifacts=process.env.SHOWCASE_OUTPUT?resolve(process.env.SHOWCASE_OUTPUT):mkdtempSync(resolve(tmpdir(),'injoffice-literal-radar-evidence-'));mkdirSync(artifacts,{recursive:true})
...
try{
 ...
 command('go',['test','-run','^TestNativeLiteralRadarSourceIntegration$','-count=1','.'],resolve(root,'go/pptxpatch'),{INJOFFICE_PPTX_RADAR_FIXTURES:artifacts})
 ...
 chrome=await launchChromeForCDP({executable,createProfile:()=>{const path=mkdtempSync(resolve(tmpdir(),'injoffice-literal-radar-chrome-'));profiles.push(path);return path}})
 ...
 for(const [index,name] of names.entries()){
  ...
  if(!response.diagnostics.some(d=>d.includes('chart.literalRadarPreview')))throw Error(name+': radar source route unavailable')
...
}catch(error){if(cdp){try{writeFileSync(resolve(artifacts,'failure.txt'),await evaluate('document.body.innerText'));const shot=await cdp.send('Page.captureScreenshot',{format:'png'});writeFileSync(resolve(artifacts,'failure.png'),Buffer.from(shot.data,'base64'))}catch{}}console.error('Literal-radar evidence: '+artifacts);throw error}finally{cdp?.close();if(chrome)await terminateProcess(chrome.child);await server?.close();if(helper)await terminateProcess(helper);for(const profile of profiles)rmSync(profile,{recursive:true,force:true,maxRetries:10,retryDelay:100});rmSync(scratch,{recursive:true,force:true})}
