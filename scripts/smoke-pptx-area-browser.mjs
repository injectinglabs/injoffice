...
try{
 ...
 for(const [index,name] of ['standard','stacked','percent','zero','large','labels','negative-stack'].entries()){
  ...
  if(name==='labels')await assert(`${literal}.textContent.includes('supplied-font')`,'labels require measured font preview')
  else {
  ...
  const fills=dataPaths.filter(n=>n.fill!=='none'),expected=name==='negative-stack'?1:name==='zero'?1:name==='standard'||name==='stacked'||name==='percent'?2:1
  ...
  if(!response.diagnostics.some(d=>d.includes('chart.literalAreaPreview')))throw new Error('missing area disclosure')
